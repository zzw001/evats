package pers.hsc.evats.core.query.parse;
...
import pers.hsc.evats.core.query.data.Condition;
import pers.hsc.evats.core.query.data.Condition.Filter;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		pers.hsc.evats.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (pers.hsc.evats.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == pers.hsc.evats.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == pers.hsc.evats.core.query.data.Sort.Direction.ASC) {
